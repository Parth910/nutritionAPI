...
const PORT = process.env.PORT || 5000;
  ...
  app.get("/",(req,res)=>{
    ...
    // }
    if(age>=1 && age<=3) res.send(mealTypeOnetoThree[randomIntFromInterval(0,2)]);
    if(age>=4 && age<=6)res.send(mealTypeFourtoSix[randomIntFromInterval(0,2)]);
    if(age>=7 && age<=9)res.send(mealTypeSeventoNine[randomIntFromInterval(0,1)]);
    if(age>=10 && age<=12)res.send(mealTypeTentoTwl[randomIntFromInterval(0,1)]);
